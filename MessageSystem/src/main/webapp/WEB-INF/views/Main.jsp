...
<body>
	...
	<div id="wrapper">
		...
		<!-- Q6. 로그인을 한 상태에서는 로그인탭 대신 로그아웃탭과 개인정보수정탭을 출력 --> <c:if
			test="${empty info}">
			<!-- empty : null과 같은 의미
				 만약에 로그인된 정보가 없다면? => 로그인을 해야한다!
			 -->
			<a href="#menu">로그인</a>
		</c:if> <c:if test="${not empty info}">
			<!-- not empty : != null
				 만약에 로그인된 정보가 있다면? => 로그아웃이 가능하다!
			-->
			<c:if test="${'admin' eq info.email }">
				<a href="showMember">회원정보수정</a>
			</c:if>
		...
		<ul class="links">
			<!-- Q3. 로그인 기능 만들기 -->
			<li><h5>로그인</h5></li>
			<!-- 요청(action) : login 전달방식 : post
									 전달 데이터 : email, password -->
		...
		<div class="inner">
			...
			<!-- ex) smhrd님 환영합니다.  --> <c:if test="${ not empty info }">
				<!-- 로그인 한 정보가 있을때 ~~님 환영합니다 -->
				<h1>${info.email}님환영합니다.</h1>
			</c:if> <c:if test="${ empty info }">
				<!-- 로그인 한 정보가 없을때 "로그린한 세션아이디 출력" -->
	...
	<script type="text/javascript">
            function checkEmail(){
               ...
               $.ajax({
                  ...
                  // 2. 보내는 데이터 (JSON타입 K:V)
                  data : {"checkEmail" : inputEmail},
